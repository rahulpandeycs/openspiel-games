...
class HSRState(object):
    """A python-only version of the HSR state.
  ...
  """
    ...
    def __init__(self, game):
        ...
        self.set_state(
            ...
            is_terminal=False,
            history=[],
            remaining_questions=_NUM_QUESTIONS,
            remaining_jars=_NUM_JARS,
            num_of_rungs=_NUM_RUNGS,
            board=np.full((_NUM_ROWS, _NUM_COLS), "."))

    # Helper functions (not part of the OpenSpiel API).

    def set_state(self, cur_player, winner, is_terminal, history, board, num_of_rungs, remaining_jars,
                  remaining_questions, hsr_guess):
        self._cur_player = cur_player
        self._winner = winner
        self._is_terminal = is_terminal
        self._history = history
        self._history.append(" HSR(2,3,7) ")
        self._remaining_questions = remaining_questions
        self._remaining_jars = remaining_jars
        self._num_of_rungs = num_of_rungs
        self._hsr_guess = hsr_guess
        self._board = board

    def coord(self, move):
        return (0, move % _NUM_COLS)

    def p_win(self):
        """Checks if a line exists, returns "x" or "o" if so, and None otherwise."""
        return self._num_of_rungs == 1

    def o_win(self):
        """Checks if the opponent has won i.e number of jars is zero, or remaining question to ask is zero,
    and number of rungs is zero."""
        return self._remaining_jars == 0 or self._remaining_questions == 0 or self._num_of_rungs == 0
    ...
    """
        ...
        if player is not None and player != self._cur_player:
            ...
            actions = []
            for action in range(1, _NUM_CELLS + 1):
                if self._board[self.coord(action - 1)] == ".":
                    actions.append(action)
            return actions
    ...
    def apply_action(self, action):
        ...
        if self.p_win():
            ...
            self._winner = 1 - self._cur_player
        elif self.o_win():
            self._is_terminal = True
            self._winner = self._cur_player
        # elif self.legal_actions(1 - self._cur_player) == []:
        #     self._is_terminal = True
        #     self._winner = self._cur_player
        else:
    ...
    def EFG_ANDR(self):
        ...
        self._num_of_rungs = _NUM_CELLS - self._hsr_guess
    ...
    def clone(self):
        cloned_state = HSRState(self._game)
        cloned_state.set_state(self._cur_player, self._winner, self._is_terminal,
                               self._history[:], np.array(self._board), self._num_of_rungs,
                               self._remaining_jars, self._remaining_questions, self._hsr_guess)
        return cloned_state


class HSRGame(object):
    """A python-only version of the HSR game.
  ...
  """
    ...
    def get_type(self):
        return pyspiel.GameType(
            short_name="python_hsr",
            long_name="Python HSR",
